...
class ConnectionManager:
    ...
    async def _handle_handshake_request(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter, message: Dict[str, Any]):
...
class NetworkTopologyManager:
    ...
    async def analyze_topology(self) -> NetworkTopology:
        ...
        try:
            ...
            if (self.topology_cache and 
                current_time - self.last_analysis < self.analysis_interval):
...
class P2PNetworkManager:
    ...
    async def _auto_connect_peers(self):
        ...
        available_peers = [
            peer for peer in self.peer_list.values()
            if (peer.peer_id not in connected_peers and 

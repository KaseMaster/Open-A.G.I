...
class HeartbeatManager:
    ...
    async def _cleanup_stale_nodes(self):
        """Limpia nodos obsoletos del registro"""
        while self.running:
            try:
                current_time = time.time()
                stale_threshold = 60.0  # 1 minuto sin heartbeat
                ...
                for node_id in stale_nodes:
                    logger.warning(f"🗑️ Eliminando nodo obsoleto: {node_id}")
                    del self.nodes[node_id]
                
                await asyncio.sleep(30)  # Limpiar cada 30 segundos
                
            except Exception as e:
                logger.error(f"❌ Error limpiando nodos obsoletos: {e}")
                await asyncio.sleep(10)
    
    async def _send_multipath_heartbeat(self, node_info: NodeInfo, heartbeat_data: Dict):
        """Envía heartbeat por múltiples rutas para mayor confiabilidad"""
        paths = [
            f"http://{node_info.address}:{node_info.port}/heartbeat",
            f"http://{node_info.address}:{node_info.port + 1}/heartbeat_backup",
            f"http://{node_info.address}:{node_info.port + 2}/heartbeat_alt"
        ]
        ...
        for path in paths:
            try:
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=3)) as session:
                    async with session.post(path, json=heartbeat_data) as response:
                        if response.status == 200:
                            success_count += 1
                            break  # Una ruta exitosa es suficiente
            except Exception:
                continue  # Intentar siguiente ruta
        
        return success_count > 0
    
    async def _initiate_node_recovery(self, node_info: NodeInfo):
        """Inicia proceso de recuperación de nodo"""
        try:
            logger.info(f"🔧 Iniciando recuperación para nodo {node_info.node_id}")
            
            # Determinar tipo de fallo
            failure_type = FailureType.TIMEOUT
            if node_info.failure_count > 5:
                failure_type = FailureType.NODE_CRASH
            
            # Delegar a recovery manager
            recovery_manager = NodeRecoveryManager(self.node_id)
            success = await recovery_manager.initiate_recovery(node_info, failure_type)
            
            if success:
                logger.info(f"✅ Recuperación exitosa de nodo {node_info.node_id}")
            else:
                logger.error(f"❌ Fallo en recuperación de nodo {node_info.node_id}")
                
        except Exception as e:
            logger.error(f"❌ Error iniciando recuperación: {e}")
    
    async def _handle_node_failure(self, node_info: NodeInfo):
        """Maneja fallo crítico de nodo"""
        try:
            logger.error(f"💀 Manejando fallo crítico de nodo {node_info.node_id}")
            
            # Migrar tareas críticas
            await self._migrate_critical_tasks(node_info)
            
            # Redistribuir datos replicados
            await self._redistribute_replicas(node_info)
            
            # Notificar a otros nodos
            await self._broadcast_node_failure(node_info)
            
        except Exception as e:
            logger.error(f"❌ Error manejando fallo de nodo: {e}")
    
    async def _migrate_critical_tasks(self, failed_node: NodeInfo):
        """Migra tareas críticas del nodo fallido"""
        try:
            # Obtener nodos disponibles
            available_nodes = [
                node for node in self.nodes.values()
                if node.status == NodeStatus.HEALTHY and node.node_id != failed_node.node_id
            ]
            
            if not available_nodes:
                logger.error("❌ No hay nodos disponibles para migración")
                return
            
            # Seleccionar nodo con menor carga
            target_node = min(available_nodes, 
                            key=lambda n: n.load_metrics.get('cpu_usage', 0))
            
            logger.info(f"📦 Migrando tareas de {failed_node.node_id} a {target_node.node_id}")
            
        except Exception as e:
            logger.error(f"❌ Error migrando tareas: {e}")
    
    async def _redistribute_replicas(self, failed_node: NodeInfo):
        """Redistribuye réplicas del nodo fallido"""
        try:
            logger.info(f"🔄 Redistribuyendo réplicas de nodo fallido {failed_node.node_id}")
            # Implementación de redistribución de réplicas
            
        except Exception as e:
            logger.error(f"❌ Error redistribuyendo réplicas: {e}")
    
    async def _broadcast_node_failure(self, failed_node: NodeInfo):
        """Notifica fallo de nodo a la red"""
        try:
            ...
            for node in self.nodes.values():
                if node.status == NodeStatus.HEALTHY:
                    await self._send_notification(node, failure_notification)
                    
        except Exception as e:
            logger.error(f"❌ Error notificando fallo: {e}")
    
    async def _send_notification(self, node: NodeInfo, notification: Dict):
        """Envía notificación a un nodo específico"""
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=3)) as session:
                url = f"http://{node.address}:{node.port}/notification"
...
class NodeRecoveryManager:
    ...
    async def _synchronize_node_data(self, node_info: NodeInfo) -> bool:
        """Sincroniza datos con nodo recuperado"""
        try:
            logger.info(f"🔄 Sincronizando datos con nodo {node_info.node_id}")
    ...
    async def _attempt_remote_restart(self, node_info: NodeInfo) -> bool:
        """Intenta reinicio remoto del nodo"""
        try:
            logger.info(f"🔄 Intentando reinicio remoto de {node_info.node_id}")
            # Implementación de reinicio remoto
            return False  # Placeholder
        except Exception as e:
            logger.error(f"❌ Error en reinicio remoto: {e}")
            return False
    
    async def _resolve_partition_conflicts(self, node_info: NodeInfo) -> bool:
        """Resuelve conflictos después de partición de red"""
        try:
            logger.info(f"🔧 Resolviendo conflictos de partición para {node_info.node_id}")
    ...
    async def _redistribute_load(self, node_info: NodeInfo) -> bool:
        """Redistribuye carga del nodo"""
        try:
            logger.info(f"⚖️ Redistribuyendo carga de {node_info.node_id}")
            # Implementación de redistribución de carga
            return True
        except Exception as e:
            logger.error(f"❌ Error redistribuyendo carga: {e}")
            return False
    
    async def _get_node_metrics(self, node_info: NodeInfo) -> Dict[str, float]:
        """Obtiene métricas actuales del nodo"""
        try:
            async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=5)) as session:
                url = f"http://{node_info.address}:{node_info.port}/metrics"
                async with session.get(url) as response:
                    if response.status == 200:
    ...
    def _is_performance_acceptable(self, metrics: Dict[str, float]) -> bool:
        """Verifica si las métricas de rendimiento son aceptables"""
        cpu_threshold = 0.8
    ...
    async def _cleanup_node_resources(self, node_info: NodeInfo) -> bool:
        """Limpia recursos del nodo"""
        try:
            logger.info(f"🧹 Limpiando recursos de {node_info.node_id}")
            # Implementación de limpieza de recursos
            return True
        except Exception as e:
...
class FaultToleranceOrchestrator:
    ...
    async def _cleanup_old_events(self):
        ...
        try:
            ...
            for node_id in list(self.active_failures.keys()):
                event = self.active_failures[node_id]
                if current_time - event.timestamp > retention_period:
                    del self.active_failures[node_id]
                    
            logger.info("🧹 Limpieza de eventos antiguos completada")
            
        except Exception as e:
            logger.error(f"❌ Error limpiando eventos: {e}")
    
    async def _optimize_replication(self):
        """Optimiza la estrategia de replicación"""
        try:
            # Analizar patrones de acceso y ajustar replicación
            logger.info("⚙️ Optimizando estrategia de replicación")
            
        except Exception as e:
            logger.error(f"❌ Error optimizando replicación: {e}")
    ...
    async def _generate_health_report(self):
        ...
        try:
            ...
            logger.info(f"📊 Reporte de salud: {healthy_nodes}/{total_nodes} nodos saludables")

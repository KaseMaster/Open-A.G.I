#!/usr/bin/env python3
"""
AEGIS CLI Main Module
Main command-line interface for AEGIS Framework
"""
...
async def start_node(dry_run: bool = False, config_path: Optional[str] = None):
    ...
    tor_mod, tor_err = safe_import("aegis.networking.tor_integration")
    p2p_mod, p2p_err = safe_import("aegis.networking.p2p_network")
    crypto_mod, crypto_err = safe_import("aegis.security.crypto_framework")
    consensus_mod, cons_err = safe_import("aegis.blockchain.consensus_algorithm")
    monitor_mod, mon_err = safe_import("aegis.monitoring.monitoring_dashboard")
    resource_mod, res_err = safe_import("aegis.optimization.resource_manager")
    perf_mod, perf_err = safe_import("aegis.optimization.performance_optimizer")
    logging_mod, logging_err = safe_import("aegis.core.logging_system")
    config_mod, config_err = safe_import("aegis.core.config_manager")
    api_mod, api_err = safe_import("aegis.api.api_server")
    metrics_mod, metrics_err = safe_import("aegis.monitoring.metrics_collector")
    alert_mod, alert_err = safe_import("aegis.monitoring.alert_system")
    dashboard_mod, dashboard_err = safe_import("aegis.api.web_dashboard")
    backup_mod, backup_err = safe_import("aegis.storage.backup_system")
...
def node_main():
    """Entry point for aegis-node command"""
    try:
        asyncio.run(start_node(dry_run=False, config_path=None))
    except Exception as e:
        logger.error(f"Fallo al iniciar el nodo: {e}")
        sys.exit(1)

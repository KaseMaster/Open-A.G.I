#!/usr/bin/env python3
"""
Full HMN Production Coherence Verification & Stabilization Test

This script performs a comprehensive verification of the Harmonic Mesh Network (HMN)
in a production WSGI environment, ensuring all components are working correctly.
"""
...
try:
    ...
    from src.models.harmonic_wallet import HarmonicWallet
    from src.models.quantum_memory import UnifiedFieldMemory
...
class HMNVerificationSystem:
    ...
    def start_wsgi_server(self) -> bool:
        """Start the WSGI production server using Waitress"""
        print("🚀 Starting WSGI production server...")
        ...
        try:
            ...
            try:
                ...
                if response.status_code == 200:
                    print("✅ WSGI server started successfully")
                    return True
                else:
                    print(f"❌ WSGI server returned status code: {response.status_code}")
                    return False
            except requests.exceptions.RequestException as e:
                print(f"❌ Failed to connect to WSGI server: {e}")
                return False
                
        except Exception as e:
            self.results.errors.append(f"Failed to start WSGI server: {str(e)}")
            print(f"❌ Failed to start WSGI server: {e}")
            return False
    
    def launch_hmn_services(self) -> bool:
        """Launch all HMN services automatically"""
    ...
    def generate_verification_report(self, performance_results: Dict[str, Any]) -> str:
        ...
        report_content = f"""
# HMN Production Coherence Verification Report
...
"""
    ...
    def run_verification(self) -> bool:
        ...
        try:
            # 1. Start WSGI production server
            if not self.start_wsgi_server():
                print("❌ Failed to start WSGI server. Aborting verification.")
                return False
            
            # 2. Launch all HMN services
            ...
            # 4. Trigger auto-balance if needed
            ...
            # 8. Verify 5-token system
            self.verify_token_system()
            
            # 9. Run stress & performance tests
    ...
    def cleanup(self):
        ...
        if self.server_process:
            try:
                ...
                print("⚠️ WSGI server force killed")
            except Exception as e:
